...
import { useRecordings, useExpressionCountByRecording } from '@/src/db/hooks';
import { Badge } from '@/src/components/ui/Badge';
import type { Recording } from '@/src/types/recording';

function RecordingItem({ item, onPress }: { item: Recording; onPress: () => void }) {
  const { colors } = useTheme();
  const styles = useMemo(() => createStyles(colors), [colors]);
  const { data: exprCount } = useExpressionCountByRecording(item.id);
  ...
  return (
    ...
    >
      ...
      <View style={styles.itemMeta}>
        ...
        <Text style={styles.metaText}>📝 {exprCount} expressions</Text>
      </View>
...
export default function RecordingsScreen() {
  ...
  const renderItem = ({ item }: { item: Recording }) => (
    <RecordingItem item={item} onPress={() => router.push(`/recording/${item.id}`)} />
  );
...
fab: { position: 'absolute', right: 24, width: 56, height: 56, borderRadius: 28, backgroundColor: colors.secondary, alignItems: 'center', justifyContent: 'center', shadowColor: 'rgba(172,53,9,0.4)', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 1, shadowRadius: 16, elevation: 8 },
});
